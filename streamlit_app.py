...
def predict_leader(image, model, reverse_class_dict):
    ...
    try:
        ...
        combined_features = np.vstack(
            (raw_features.reshape(-1, 1), haar_features.reshape(-1, 1))
        ).flatten()
        ...
        try:
            ...
            max_confidence = np.max(confidence_proba) * 100
        except Exception:
        ...
        # Return combined_features so the chart block can use it
        return predicted_name, max_confidence, combined_features
...
# Main header
st.markdown(
    "<h1 class='main-header'>🎭 Ethiopian Leaders Image Classification Model</h1>",
    unsafe_allow_html=True
)
    ...
    with cols[idx]:
        ...
        if os.path.exists(image_path):
            try:
                ...
                st.markdown(
                    f"<p style='text-align:center;font-weight:bold;'>{leader_name}</p>",
                    unsafe_allow_html=True
                )
            except Exception:
...
if uploaded_file is not None:
        ...
        with st.spinner("Processing image..."):
            ...
            if cropped_image[0] is not None:
                cropped_img_pil = Image.fromarray(
                    cv2.cvtColor(cropped_image[0], cv2.COLOR_BGR2RGB)
                )
    ...
    if st.button("🎯 Predict Leader", use_container_width=True, type="primary"):
        cropped_image_data, status = get_cropped_image_if_2_eyes(
            temp_image_path, face_cascade, eye_cascade
        )[:2]

        if cropped_image_data is not None:
            with st.spinner("Making prediction..."):
                # Now receives combined_features as third return value
                ...
                if predicted_leader is not None:
                    if confidence > 94:
                        ...
                        st.markdown(f"<p style='font-size: 18px; color: #856404;'>Model confidence: {confidence:.2f}% (Threshold: >94%)</p>", unsafe_allow_html=True)
                    ...
                    st.markdown(
                        ...
                        f"padding:20px;border-radius:5px;margin-top:20px;'>"
                        f"<p style='font-size:18px;color:#856404;'>"
                        f"Confidence: {confidence:.2f}% (Threshold: >94%)</p>"
                        f"<p style='color:#856404;'>This person may not be one of the trained leaders.</p>"
                        f"</div>",
                        unsafe_allow_html=True
                    )

                # Per-leader probability bar chart
                st.markdown(
                    "<h4 style='color:#1f77b4;margin-top:30px;'>📊 Confidence per Leader</h4>",
                    unsafe_allow_html=True
                )
                try:
                    ...
                    leader_names = [reverse_class_dict[i] for i in range(len(proba))]
                    sorted_pairs = sorted(
                        zip(leader_names, proba), key=lambda x: x[1], reverse=True
                    )
                    names_sorted, probs_sorted = zip(*sorted_pairs)
                    colors = [
                        '#2ca02c' if n == predicted_leader else '#1f77b4'
                    ...
                    ax.set_title('Model Confidence per Leader', fontsize=12,
                                 fontweight='bold', pad=10)
                    ax.axvline(x=94, color='red', linestyle='--', linewidth=1,
                               label='94% threshold')
